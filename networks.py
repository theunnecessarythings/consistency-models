...
#----------------------------------------------------------------------------
# Preconditioning corresponding to the Consistency Training (CT) from the paper "Consistency Models"


class CTPrecond(torch.nn.Module):
    def __init__(self,
        cfg,
        label_dim       = 0,            # Number of class labels, 0 = unconditional.
        use_fp16        = False,        # Execute the underlying model at FP16 precision?
    ...
    def t(self, i, k):
        ...
        t_steps = torch.cat([torch.zeros_like(t_steps[:1]), self.round_sigma(t_steps)])  
    ...
    def N(self, k):
        ...
        return int(np.ceil(np.sqrt(k/K * ((s1 + 1)**2 - s0**2) + s0**2) - 1) + 1)
